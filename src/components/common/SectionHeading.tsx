...
export function SectionHeading({ kicker, title, subtitle, align = 'center', className }: SectionHeadingProps) {
  return (
    <div className={cx('mx-auto max-w-3xl', align === 'center' ? 'text-center' : 'text-left', className)}>
      {kicker && <p className="text-xs font-semibold uppercase tracking-[0.3em] text-emerald-600">{kicker}</p>}
      <h2 className="mt-3 text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl lg:text-5xl">{title}</h2>
